...
- из формы проекта убраны внутренние поля `role` и `sort_order`;
- порядок проекта назначается backend автоматически при создании;
- добавлен отдельный CRUD-раздел для технологий;
- технологии, связанные с защищёнными проектами, нельзя изменить или удалить через demo-админку;
- добавлены feature tests для управления технологиями.
- slug проекта убран из формы и генерируется автоматически на backend;
- блок изображений проекта заменён на drag and drop галерею;
- выбранные изображения добавляются в общий список, а не заменяют предыдущий выбор;
- для каждого изображения можно изменить описание, удалить его, поднять/опустить в списке и сделать главным;
- порядок и главное изображение сохраняются через `sort_order`.
