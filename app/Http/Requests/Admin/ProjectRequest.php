...
final class ProjectRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            'title.max' => 'Название проекта не должно быть длиннее 255 символов.',
            'slug.unique' => 'URL-адрес для этого названия уже занят.',
            'description.required' => 'Добавьте описание проекта.',
            'description.max' => 'Описание не должно быть длиннее 5000 символов.',
            'problem.max' => 'Описание задачи не должно быть длиннее 5000 символов.',
            'solution.max' => 'Описание решения не должно быть длиннее 5000 символов.',
            'result.max' => 'Описание результата не должно быть длиннее 5000 символов.',
            'website_url.url' => 'Укажите корректную ссылку на сайт.',
            'website_url.max' => 'Ссылка на сайт слишком длинная.',
            'repository_url.url' => 'Укажите корректную ссылку на репозиторий.',
            'repository_url.max' => 'Ссылка на репозиторий слишком длинная.',
            'started_at.date' => 'Укажите корректную дату начала.',
            'finished_at.date' => 'Укажите корректную дату завершения.',
            'finished_at.after_or_equal' => 'Дата завершения не может быть раньше даты начала.',
            'technologies.*.exists' => 'Выбранная технология не найдена.',
            'images.*.path.required' => 'Для существующего изображения не найден путь.',
            'images.*.alt.max' => 'Описание изображения не должно быть длиннее 255 символов.',
            'images.*.sort_order.required' => 'Укажите порядок изображения.',
            'images.*.sort_order.distinct' => 'Порядок изображений не должен повторяться.',
            ...
            'uploaded_images_meta.*.alt.max' => 'Описание изображения не должно быть длиннее 255 символов.',
            'uploaded_images_meta.*.sort_order.required_with' => 'Для каждого изображения должен быть указан порядок.',
        ];
    }
